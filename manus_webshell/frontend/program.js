...
import template from "./templates/program.handlebars";
...
function initializeEditor() {
  ...
  $.ajax('/assets/blockly-toolbox.xml').done(function(toolbox) {

    var body = $(template());

    var blocklyContainer = body.find(".blockly");
    var blocklyArea = blocklyContainer.find('.area')[0];
    var blocklyDiv = blocklyContainer.find('.workspace')[0];
    blocklyContainer.append(Blockly.Xml.domToText(toolbox));

    Program.blockly = {};

    Program.running_app = null;
    Program.blockly.workspace = Blockly.inject(blocklyDiv,{
        grid:{
            spacing: 25,
            length: 3,
            colour: '#ddd',
    ...
    var editorContainer = body.find(".editor")[0];

    Program.python = {};
    Program.python.shown = false;
    Program.python.in_use = false;
    Program.python.ace = ace.edit(editorContainer);
  ...
  var console = body.find(".console");
  var consoleContent = console.find(".content");
  var consoleInput = console.find(".inputline");
  ...
  body.find(".toolbar.left").append(run_button).append(stop_button).append(new_program).append(convert_python);
  body.find(".toolbar.right").append(save_button).append(load_button);
  ...
  PubSub.subscribe("apps.console", function(msg, data) {
    if (data.identifier == Program.running_app) {
      for (i in data.lines) {
        consoleContent.append($("<span>").addClass(data.source).text(data.lines[i]));
      }
    }
  });
}
...
Program = {
    ...
    Interface.dialog("Load program", 
      function(container) {
        var onclose = Program._filemanager(container, "loading", function(key, name) {
          Storage.get(key, function(key, data) {
  ...
  _filemanager: function(container, type, callback) {
    ...
    Storage.list(function(keys) {

        for (var i in keys) {
          if (!keys[i].startsWith("program_"))
            continue;
          Storage.get(keys[i], function(key, value) {
            value["key"] = key;
            list.add([value]);
    ...
    var subscribtions = {
      update : PubSub.subscribe("storage.update", function(msg, key) {
          ...
          Storage.get(key, function(key, value) {
  ...
  update_state : function() {
        ...
        var blockly_visible = Program.blockly .is(":visible");
    ...
    save_current_as_read_only: function(name){
        ...
        Storage.set(key, {language: curr.language,
            name: name,
            timestamp: timestamp,
            code: curr.code,
            read_only: true});
